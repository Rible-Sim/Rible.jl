...
function inversekinematics(θs;k,c)
    ...
    for θ in θs
        ...
        RB.actuate!(man_act,a)
        rls = [s.state.restlen for s in man_act.st.cables]
        push!(as,rls)
    end
    manipulator, as
end
...
function eigenanalysis(θs; k, c)
    ...
    for θ in θs
        ...
        RB.actuate!(actmani,ai)
        ωi,Zi = RB.undamped_eigen!(actmani.st,refqi,refλi)
        push!(ωs,ωi)
        push!(Zs.u,Zi)
        push!(as.u,ai)
    end
...
function pyplot_mode(tgstruct,θ,Z,xlim,ylim)
    function plot2ax!(ax,tgstruct,q;ref=false)
        RB.distribute_q_to_rbs!(tgstruct,q)
