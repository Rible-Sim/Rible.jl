...
function rigidbody(i, rdsi, r̄si, theta; nb=nb)
    if i % nb == 1
        contactable = false
        visible = true
        ci = Int[]
...
function rigidSphere(pos, radius, id)
    ...
    ri = pos
    aps = [[radius 0.0 0.0] for _ in 1:20]
...
rbs1 = [rigidbody(i, rds[i], r̄s[i], 0.0) for i in 1:nb]
rbs2 = [rigidbody(i + nb, rds[i], r̄s[i], pi / 2) for i in 1:nb]
rbs3 = [rigidbody(i + 2nb, rds[i], r̄s[i], pi) for i in 1:nb]
rbs4 = [rigidbody(i + 3nb, rds[i], r̄s[i], 3pi / 2) for i in 1:nb]
# rbs2 = [rigidSphere([1.8, 0.0, -0.45], 0.2, length(rbs1)+1)]
rbs = vcat(rbs1, rbs2, rbs3, rbs4)
