...
function update_scene!(ax,tg,cube_bars,q)
    # TR.reset_forces!(tg)
    TR.update_rigids!(tg,q)
    newcube, newbars = get_cube_bars(tg)
    cube, bars = cube_bars
    cube[] = newcube
    bars[] = newbars

    # afcs,get_gaps,update_contacts = confuncs(tg)
    # update_contacts(q,get_gaps(q))
    # activecontacts = [afc.impulse.active for afc in afcs]
    # @show activecontacts
end

function sliderplot(fig,tg,bot,ax,cube_bars,contact_dynamics)
    # 𝒈,get_indices,,get_contacts = contact_dynamics(tg)
    (;traj) = bot
    ...
    on(sg.sliders[1].value) do this_step
        ...
        q = traj.q[this_step]
        update_scene!(ax,tg,cube_bars,q)
        # @show get_contacts(q)
...
#     update_scene!(scene,sol.qs[istep],tg,cube)
# end
...
function vis(bot,contact_dynamics;do_record=false)
    ...
    cube_bars = vis!(ax,tg,get_cube_bars(tg))
    ...
    if do_record
        ...
        sliderplot(fig,tg,bot,ax,cube_bars,contact_dynamics)
    end
    fig
end
