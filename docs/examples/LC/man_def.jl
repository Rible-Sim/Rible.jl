function man_nd(n;θ=0.0,k=0.0,c=0.0)
    ...
    ss = [TR.SString2D(original_restlens[i],ks[i],cs[i]) for i = 1:ncables]
    acs = [
    ...
    for i = 1:n-1
        j = 2i - 1
        push!(string2ap,(TR.ID(j,3),TR.ID(j+2,1)))
        push!(string2ap,(TR.ID(j,2),TR.ID(j+2,3)))
        push!(string2ap,(TR.ID(j,4),TR.ID(j+2,1)))
        push!(string2ap,(TR.ID(j,2),TR.ID(j+2,4)))
    end
