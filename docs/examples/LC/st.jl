...
# for setting c
# c = TR.get_c(twodofbot)
# TR.set_C!(twodofbot.tg,c)
...
function locate_bifurcation_point(bot_input,γ0)
    ...
    q0 = TR.get_q(tg)
    c0 = TR.get_c(tg)
    ℓ0 = TR.get_cables_len(tg)
    s0 = 1 ./ℓ0
    k0 = TR.get_cables_stiffness(tg)
    μ0 = TR.get_cables_restlen(tg)
    _,λ0 = TR.check_static_equilibrium_output_multipliers(tg)
    Φ = TR.make_Φ(tg,q0)
    A = TR.make_A(tg,q0)
    Q̌ = TR.make_Q̌(tg,q0)
    S = TR.make_S(tg,q0)
    Ǩm_Ǩg = TR.make_Ǩm_Ǩg(tg,q0)
    # state variables
...
pfrc = TR.recover(pf,twodofbot.tg)
TR.set_new_initial!(twodofbot_plot,pfrc.q)
...
q0,q̇0,λ0 = TR.get_initial(tg)
M = TR.build_massmatrix(tg)
A = TR.build_A(tg)
u0 = [s.original_restlen for s in tg.cables]
K = diagm([s.k for s in tg.cables])
J = [TR.build_Ji(tg,i) for i = 1:4]
