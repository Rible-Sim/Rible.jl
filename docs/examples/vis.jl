...
function init_plot!(ax,tgob;isref=false,showlabels=true,textsize=10)
    ...
    # rigid bars
    rigidbars = TR.get_rigidbodies(tgob[])
    # rigidbars = TR.get_rigidbodies(tgob[])
    # @show length(rigidbars)
    if !isempty(rigidbars)
        linesegs_bars = @lift begin
            # bars = TR.get_rigidbars($tgob)
            bars = TR.get_rigidbodies($tgob)
            ndim = TR.get_ndim($tgob)
            T = TR.get_numbertype($tgob)
            ret = Vector{Pair{Point{ndim,T},Point{ndim,T}}}()
            mapreduce(
                (bar)-> @match length(bar.state.rps) begin
                    2 => Point(bar.state.rps[1]) => Point(bar.state.rps[2])
                    3 => Point(bar.state.rps[1]) => Point(bar.state.rps[2])
                    4 => [Point(bar.state.rps[1]) => Point(SVector{2}(0.0, 0.0)),
                          Point(bar.state.rps[2]) => Point(SVector{2}(0.0, 0.0)),
                          Point(bar.state.rps[3]) => Point(SVector{2}(0.0, 0.0))]
                    5 => [Point(bar.state.rps[1]) => Point(bar.state.rps[5]),
                          Point(bar.state.rps[2]) => Point(bar.state.rps[5]),
                          Point(bar.state.rps[3]) => Point(bar.state.rps[5]),
                          Point(bar.state.rps[4]) => Point(bar.state.rps[5])]
                end
                ,
                vcat,
                bars
