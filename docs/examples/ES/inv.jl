...
function inv!(bots;)
    for (i, bot) in enumerate(bots)
        μ = TR.inverse_for_restlength(bot, bot)
        TR.set_restlen!(bot.tg,μ)
        TR.update!(bot.tg)
        # κ = TR.get_cables_stiffness(bot.tg)
        # ℓ = TR.get_cables_len(bot.tg)
        # push!(ret,μ)
        if i > 1
            push!(bots[1].traj,bot.tg.state.system)
        end
    end
    bots
end
...
function plotsave_inv_restlen(bots,xs,figname=nothing;
    ...
    μs = TR.get_cables_restlen.(bots) |> VectorOfArray
    fs = TR.get_cables_tension.(bots) |> VectorOfArray
