...
function make_bai(meshes,质心,质量,惯量)
    ...
    function rigidbody(i,P)
        ...
        mesh_rigid = meshes[i]
        rb = TR.RigidBody(prop, state, mesh_rigid)
        rb
    end
    rbs = [
    ...
    cables =
        [TR.Cable2D(i, original_restlens[i], ks[i], 0.0;slack=true) for i = 1:ncables]  #
...
# 静力平衡
TR.check_static_equilibrium_output_multipliers(tail.tg;)

# 刚度、稳定性分析
TR.undamped_eigen(tail.tg)
