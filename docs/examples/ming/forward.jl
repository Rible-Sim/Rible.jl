...
# Φ = TR.build_Φ(spine2.tg)
# q, _ = TR.get_q(spine2.tg)
# Φ(q)
# @code_warntype Φ(q)
# A = TR.build_A(spine2.tg)
# A(q)
# @code_warntype A(q)
...
function get_seqs(bot,rbid,pid,f=[1.0,0.0,0.0],g2=[5.0])
    ...
    start_sol,parameters0 = TR.get_start_system(bot,TR.DeformMode())
    @show inv.(start_sol.s)
    d0, u0, g0 = parameters0
    u1 = TR.get_cables_restlen(bot)
    @show u0
    @show u1
    # d1 = TR.get_d(bot.tg)
    ...
    F̌ = TR.build_F̌(bot.tg,rbid,pid,f)
    # sys,_ = TR.forward_system(bot.tg,TR.DeformMode();F̌)

    # seq = TR.forward_once(bot.tg,start_sol,parameters0,parameters1,TR.DeformMode();F)
    # seq = TR.forward_sequence(bot_sys,start_sol,start_parameters,target_parameters)
    seqs = TR.forward_multi_sequence(bot.tg,start_sol,parameters_seq,TR.DeformMode();F̌,n=10)
    # @btime TR.forward_multi_sequence($sys,$start_sol,$parameters_seq;n=1)
    # seqs[1]
...
TR.apply!(spine2,seqs[3])
plot_traj!(spine2;)

# @code_warntype get_seqs(spine2,rbid,pid,f)
seqs
qs = [seqs[2][begin].q,seqs[3][begin].q,seqs[3][end].q]
TR.set_new_initial!(spine2,qs[1])
TR.get_cables_len(spine2)
h = spine2.tg.rigidbodies[2].state.rps[pid][3] - spine2.tg.rigidbodies[1].state.rps[pid][3]
h
...
function plot_force_arrow!(ax,tg,rbid,pid,f,font_pixel)
    rbs = tg.rigidbodies
    rb = rbs[rbid]
...
function plot2(bot,qs,rbid,pid,f)
    ...
    # bars1,cables1 = plotstructure!(ax1,tg)
    bars2,cables2 = plotstructure!(ax1,tg)
    bars3,cables3 = plotstructure!(ax2,tg)
    # update_scene!(tg,bars1,cables1,qs[1])
    update_scene!(tg,bars2,cables2,qs[2])
    plot_force_arrow!(ax1,tg,rbid,pid,f,font_pixel)
    update_scene!(tg,bars3,cables3,qs[3])
    plot_force_arrow!(ax2,tg,rbid,pid,f,font_pixel)
    # set_axis!(ax1)
    set_axis!(ax1)
    set_axis!(ax2)
    ...
    # sequenceplot(fig,grid_tg,ax,tg,bars,cables,update_scene!,seq,dlabels,ulabels,glabels)
    colsize = 500
    rowsize = 480
    for i = 1:2
...
function plot_graph(bot,seqs,pid,figname=nothing)
    ...
    u2 = get_trajectory!(bot,2,pid)

    d = [d[7] for d in seqs[3].d]
    TR.set_new_initial!(bot,seqs[3].q[1])
    append!(traj.q, seqs[3].q[2:end])
    u3 = get_trajectory!(bot,2,pid)
...
TR.set_new_initial!(spine2_video,seqs[1][end].q)
seq_sol = seqs[1][end]

start_sol,parameters1 = TR.get_start_system(spine2_video,TR.build_Y(spine2),TR.DeformMode())
parameters2 = deepcopy(parameters1)
parameters2.g[1] = 5.0
F = TR.build_F(spine2.tg,rbid,pid,f)
seq2 = TR.forward_sequence(spine2_video.tg,start_sol,parameters1,parameters2,TR.DeformMode();F)
...
function plot_video(bot,seq_sol,rbid,pid,f)
    ...
    # ax1 = Axis3(fig[1, 1]; aspect = :data, viewmode = :fit, azimuth, elevation)
    ax1 = fig[1, 1] = LScene(fig, scenekw = (camera = cam3d!, raw = false))
    bars,cables = plotstructure!(ax1,bot.tg)
    ...
    F = TR.build_F(bot.tg,rbid,pid,f)
    d0,u0,g0 = parameters0
    ...
    onany(targets_ob...) do targets...
        ...
        result =  TR.forward_once(bot.tg,start_sol,parameters0,parameters1,TR.DeformMode();F)
        update_scene!(bot.tg,bars,cables,result.q)
...
function compare(nb)
    ...
    f = [5.0,0.0,0.0]
    F = reshape(TR.build_F(spine_initial_DR.tg,nb,pid,f),:,1)
    ...
    ################### Testing HC #########################
    # @info "Testing HC"
    # start_sol,parameters0 = TR.get_start_system(spine_initial_DR,TR.PrimalMode())
    # u0, g0 = parameters0
    # # @show u0
    # u1 = TR.get_cables_restlen(spine_initial_DR)
    # # @show u1
    # parameters1 = (u=u1, g=g0)
    # parameters2 = (u=u1, g=[1.0])
    # @info "Precompiling"
    # sol1 = TR.forward_once(spine_initial_DR.tg,start_sol,parameters0,parameters1,TR.PrimalMode();F)
    # @info "Benchmarking"
    # sol2 = @btime $TR.forward_once($spine_initial_DR.tg,$sol1,$parameters1,$parameters2,$TR.PrimalMode();F=$F)
...
TR.set_new_initial!(spine_initial_DR,spine_initial_DR.traj.qs[end])
...
TR.set_new_initial!(spine_compare,time_forward.value.q)
...
plot(rs,axis = (yscale = log10,))
@time GDR(spine2.tg;maxiters=1000,ϵ=1e-14)
@descend_code_warntype GDR(spine2.tg;maxiters=1000,ϵ=1e-14)o
includet("tsff.jl")
TR.get_cables_len(spine2)
errs,Ns = tsff(spine2.tg)
plot(errs)
errs[end]
@time tsff(spine2.tg)
@descend_code_warntype tsff(spine2.tg)
[rb.state.ro for rb in spine2.tg.rigidbodies]
