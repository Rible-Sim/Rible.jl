...
c_section = StructArray([TR.CableSegment(i, lens[i], .2, prestress=pre[i]) for i in 1:ncsegs])
cs1 = TR.ClusterCables(1, ncsegs-1, deepcopy(c_section); μ=0.02)
cs2 = TR.ClusterCables(2, ncsegs-1, deepcopy(c_section); μ=0.02)
...
# bot.traj.q̇[begin][bot.tg.connectivity.indexed.mem2sysfull[end][1:4]] .= [50,0.0,50,0.0]
...
# plot_traj!(bot_plot)
temp = Matrix(TR.build_M̌(bot.tg))
