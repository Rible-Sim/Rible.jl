...
function parse_material(T,material)    
    ...
    TR.URDF.Material(name,color,texture)
end
        ...
        begin
            ...
            if visual !== nothing
                ...
                if nodename(g) == "mesh"
                    ...
                    visual_mesh_geometry = TR.URDF.MeshGeometry(
                        fileobj,
                        scale
                    )
                else
...
function link_to_rigidbody(i,link::TR.URDF.Link, 
    ...
    @show link.name, ro, R
    TR.RigidBody(prop,state,rigidmesh)
end
...
function parse_joints_links!(
        joints_input::AbstractVector{<:TR.URDF.Joint{T}},
        linksdict
    ) where T
...
function urdf_to_rigidrobot(urdf,T=Float64)
	...
	numbered = TR.number(rbs)
	indexed = TR.index(rbs,)
    ss = Int[]
	tensiles = (cables = ss,)
	connected = TR.connect(rbs,zeros(Int,0,0))
	tensioned = @eponymtuple(connected,)
    jointed = TR.join(TypeSortedCollection(jointcsts),indexed)
...
starsimage = load(TR.assetpath("stars.jpg")) |> rotr90


q = TR.get_q(newbot.tg)
Φ = TR.make_Φ(newbot)
