...
bars,cables = bars_and_cables_segs(manipulator.tg)
...
function simulate_free(ndof;k,c,unit="mks",dt=0.01,tend=10.0,verbose=false)
    ...
    prob = TR.DyProblem(free(manipulator),manipulator,(0.0,tend))

    TR.solve!(manipulator,prob,TR.Zhong06();dt,ftol=1e-14,verbose)
    manipulator
end
...
l0s = TR.get_cables_len(man2)

function pyplot_energy(tr,l0s)
    @unpack tg, traj = tr
    # res = TR.AdamsResults("1BWI3E11H3.xml")
    # t = res("time")
    ts = traj.ts
