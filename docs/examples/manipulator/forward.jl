...
function dist!(tg,state)
    target = GeometryBasics.Circle(Point2(0.061260,-0.21),0.1)
    T = TR.get_numbertype(tg)
    TR.update_points!(tg,state.c)
    TR.update_rigids!(tg,state.q)
    d = Ref(zero(T))
...
function get_actuate_seqs(bot,g0=[0.0])
    ...
    P,variable_groups,parameters = TR.forward_system(tg,TR.AllMode();)
    ...
    function f_core(x)
        d1[indx_d], c1[indx_c] = TR.split_by_lengths(x,xlens)
        parameters1 = (d=d1, c=c1, k=k0, u=u1, g=g1)
        seq_raw = TR.forward_sequence(Psys,
                            startsols,
                            parameters0,
                            parameters1,
                            ide_pindx;n=1)
        seq_rc = TR.recover.(seq_raw,Ref(tg))
    end
    function f(x)
        seq_rc = f_core(x)
...
TR.apply!(man_inv_plot,seqs_diff)
dist!(man_inv_plot.tg,seqs_diff[end])
...
TR.apply!(man_inv_plot,StructArray([seq1]))
