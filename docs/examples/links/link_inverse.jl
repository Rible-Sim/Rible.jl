...
function inverse_analysis(θs)
    ...
    for i = 1:n
        ...
        # actlinkn,lhs,rhs = TR.statics_equation_for_actuation(linkn,reflinkn,Y,gravity=false,scale=false)
        ...
        refλi,_,ai= TR.inverse_for_actuation(linkn,reflinkn,Y,gravity=false,scale=false)
...
function inverse2energy(tgstruct,refstruct,Y)
    ...
    for (i,ξ) in enumerate(LinRange(0,ξ_max,nξ))
        ...
        TR.actuate!(actstruct,ai)
        epei = TR.elastic_potential_energy(actstruct)
...
function inverse_with_energy_3(tr,reftr,Y,epes)
    acttg,lhs,rhs,c = TR.statics_equation_for_actuation(tr.tg,reftr.tg,Y,gravity=false,scale=false)
    acttr = TR.TensegrityRobot(acttg,deepcopy(tr.hub))
    xp,nb = TR.get_solution_set(lhs,rhs)
    ...
    TR.actuation_check(acttr,Y,a_result)
    refq,_ = TR.get_q(reftr.tg)
    TR.actuate!(acttr,a_result)
    @assert TR.check_static_equilibrium(acttg,refq,λ_result)
    u0 = TR.get_original_restlen(acttr)
    rl_result = u0 + Y*a_result
    λ_result,rl_result,a_result
end
