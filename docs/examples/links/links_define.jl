...
function links(n,di,rm;k=3e1,c=0.0)
    ...
    function rigidbody(i,prop,aps,ro,R,ṙo,ω)
		...
		# lncs,q,q̇ = TR.NCF.NC2P2V(ri,rj,rk-ri,rl-ri,ro,R,ṙo,ω)
    ...
    ss = [TR.SString3D(i,stringlens[i],ks[i],cs[i]) for i = 1:nstrings]
	tensiles = (strings=ss,)
	acs = [TR.ManualActuator(TR.SimpleRegistor(6(i-1)+j,stringlens[6(i-1)+j])) for i = 1:n-1  for j = 1:6]
	hub = (actuators=acs,)
