...
function simulate_linkn(;dt=0.01,k=3e1,c=0.0,tend=10.0,verbose=false)
    ...
    refq0,refq̇0,refλ0 = TR.get_initial(reflinkn.tg)
    TR.set_new_initial!(linkn,refq0,refq̇0)

    prob = TR.SimProblem(linkn,dynfuncs,(0.0,tend))
...
bars,strings = bars_and_strings_segs_3D(reflinkn.tg)
...
u0 = TR.get_original_restlen(linkn_undamped)
plotstructure(linkn_undamped)
...
function plot_trajectory(tr)
    ...
    ts = traj.ts
    res = TR.AdamsResults("2AWI3E14H3.xml")
...
l0s = TR.get_strings_len(reflinkn)
resE = TR.AdamsResults("2B49G.xml")
tE = resE("time")
resE("P2_CM")
function pyplot_energy(tr,l0s;gravity=false)
    @unpack tg, traj = tr
    if gravity
        resH = TR.AdamsResults("2BGWI3E3H3.xml")
        resE = TR.AdamsResults("2B49G.xml")
    else
        resH = TR.AdamsResults("2BWI3E14H3.xml")
        resE = TR.AdamsResults("2B49.xml")
    end
    tH = resH("time")
    tE = resE("time")
    ts = traj.ts
