...
function Tbars(;θ = 0, coordsType = :NCF)
    ...
    function make_base(i)
        ...
        loci_positions = [r̄p1,r̄p2,r̄p3,r̄p4,r̄p5,r̄p6,r̄p7]
        axes = [
        ...
        if coordsType == :NCF
            ...
            qcs = QCF.QC(m,Ī)
            pres_idx = collect(1:7)
            cstr_idx = Int[]    
            coords = RB.PresFreeCoordinates(qcs, pres_idx)
        end
        basemesh = load(RB.assetpath("assembly1.STL")) |> RB.make_patch(;
            # trans=[-1.0,0,0],
            rot = RotZ(π),
            scale=1/500,
            color = :silver,
    ...
    function make_slider(i;ri=SVo3,R=RotZ(0.0),)
        ...
        if coordsType == :NCF
            nmcs = RB.NCF.NC1P3V(ri, ro, R)
            coords = RB.PresFreeCoordinates(nmcs,)
        else
            qcs = QCF.QC(m,Ī)
            coords = RB.PresFreeCoordinates(qcs,)
        end
        slidermesh = load(RB.assetpath("assembly2_2.STL")) |> RB.make_patch(;
            # trans=[-1.0,0,0],
            rot = begin
                if i == 2
                    RotYZ(π,π/2)
    ...
    cables = RibleTensegrity.connect_spring(rigdibodies, spring_dampers; cm,)


    j1 = RB.PrismaticJoint(ncables+1,RB.Hen2Egg(RB.Anchor(base,5,1),RB.Anchor(slider1,1,1)))
    j2 = RB.PrismaticJoint(ncables+2,RB.Hen2Egg(RB.Anchor(base,5,2),RB.Anchor(slider2,1,1)))

    j3 = RB.PinJoint(ncables+3,RB.Hen2Egg(RB.Anchor(bar,1,1),RB.Anchor(slider1,2,1)))
    j4 = RB.PinJoint(ncables+4,RB.Hen2Egg(RB.Anchor(bar,2,1),RB.Anchor(slider2,2,1)))
    ...
    #     0 0 3 6;
    # ]
    cnt = RB.PresFreeConnectivity(rigdibodies,apparatuses)
