struct AlphaCache{solverT,cacheType}
    solver::solverT
    cache::cacheType
end
...
function generate_cache(
    ...
    (;αm,αf,γ,β) = solver.integrator
    F!(F,q,q̇,t) = gen_force!(F,bot,env.field,policy,q,q̇,t;)
    Jac_F!(∂F∂q̌,∂F∂q̌̇, q,q̇,t) = gen_force_state_jacobian!(∂F∂q̌,∂F∂q̌̇,bot,env.field,policy,q,q̇,t)
    (;M,M⁻¹,M̌,M̌⁻¹,Ḿ,M̄)= build_mass_matrices(structure)
    # (;M) = mm
    A = make_cstr_jacobian(bot)
    Φ = make_cstr_function(bot)

    ∂Ǎᵀλ∂q̌ = (q,λ)->cstr_forces_jacobian(bot.structure,q,λ)
...
function solve!(sim::Simulator,cache::AlphaCache;
    ...
    (;bot,) = prob
    (;traj) = bot
    (;F!,Jac_F!,A,Φ,∂Ǎᵀλ∂q̌,Ḿ,M̌,M̄,M̌⁻¹,β′,γ′) = cache.cache
    (;αm,αf,γ,β) = cache.solver.integrator
    q̌0 = traj.q̌[begin]
    λ0 = traj.λ[begin]
    q0 = traj.q[begin]
    q̇0 = traj.q̇[begin]
