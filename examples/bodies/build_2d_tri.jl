function build_2d_tri(id, ri, rj=nothing, rk=nothing;
    ...
    axes = [SVector{2}([1.0, 0.0]) for _ in eachindex(loci_positions)]
    Īgx = (b * h^3) / 36
    ...
    trimesh = begin
        if id == 1
        load(RB.assetpath("part1_copy.STL")) |> RB.make_patch(; scale=1 / 1000, color=:mediumpurple4)
        else
        load(RB.assetpath("part1.STL")) |> RB.make_patch(; scale=1 / 1000, color=:slategray4)
        end
    end
