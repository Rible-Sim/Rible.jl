function make_tail(n)
    ...
    ss = [TR.SString2D(0.5original_restlens[i],ks[i],10.0) for i = 1:nstrings]
...
function make_curve_tail(n)
    ...
    function rigidbody(i,CoM,m,inertia,ri,rj,vi,vj,aps)
        if i == 0
            movable = true
            constrained = true
    ...
    rbs = [rigidbody(i,CoM[i],m[i],inertia[i],
                    ri[i],rj[i],vi[i],vj[i],
    ...
    ss = [TR.SString2D(0.5original_restlens[i],ks[i],0.0) for i = 1:nstrings]
    # @code_warntype   TR.DString(k[i],original_restlen[i],
    #         restlen[i],actuallength[i],zeros(MVector{4}))
