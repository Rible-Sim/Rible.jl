...
@inline function LinearAlgebra.cross(a::Number,b::AbstractVector)
    ret = similar(b)
    ret[1] = -a*b[2]
    ret[2] =  a*b[1]
    ret
...
@inline function LinearAlgebra.cross(a::StaticArray{Tuple{1}, T, 1},b::StaticArray{Tuple{2}, T, 1}) where T
    ret = similar(b)
    ret[1] = -a[1]*b[2]
    ret[2] =  a[1]*b[1]
    ret
...
@inline function HouseholderOrthogonalization(n)
    n̂ = norm(n)
    _, i = findmax(n.+n̂)
    ei = 1:3 .== i
    h = n + n̂*ei
...
function (f::FischerBurmeister)(x,y)
    (;ϵ,X₁,X₂) = f
    √((X₁*x)^2+(X₂*y)^2+2ϵ^2) - (X₁*x+X₂*y)
end

struct FischerBurmeister_first_derivatives{T}
    ϵ::T
    X₁::T
    X₂::T
end

function (f::FischerBurmeister_first_derivatives)(x,y)
    (;ϵ,X₁,X₂) = f
    den = √((X₁*x)^2+(X₂*y)^2+2ϵ^2)
    ∂x = X₁^2*x/den - X₁
    ∂y = X₂^2*y/den - X₂
    @eponymtuple(∂x,∂y)
end
