function build_joint_cache(
    ...
    r_hen = to_position(coords_hen,q_hen,c_hen)
    r_egg = to_position(coords_egg,q_egg,c_egg)
    d = r_egg - r_hen
    # cstr 1st
    vio_1st = d
    vio_2nd = [d'*d]
...
function make_cstr_jacobian(
    ...
    function inner_cstr_jacobian(q)
        ...
        # @show quat_trl_egg
        # @show quat_rot_egg
        C_hen = to_position_jacobian(coords_hen,inner_q_hen,c_hen)
        C_egg = to_position_jacobian(coords_egg,inner_q_egg,c_egg)
        J = [-C_hen C_egg;] 
        r_hen = to_position(coords_hen,inner_q_hen,c_hen)
        r_egg = to_position(coords_egg,inner_q_egg,c_egg)
        d = r_egg - r_hen
        # hes 1st
        # jac 1st 
        inner_jac_1st[1:3,:] .= J
...
function add_joint_forces_jacobian!(
        ret,
        num_of_cstr,
        coords_hen::QC, coords_egg::QC,
        loci_position_hen,
